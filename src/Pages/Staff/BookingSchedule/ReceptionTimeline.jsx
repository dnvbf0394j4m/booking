...
// ✅ axios instance có interceptor (refresh token tự động)
import api from "../../../api/client";

// ✅ các API helper dùng api bên trong (không cần token)
import {
  fetchRooms,
  fetchBookingsRange,
  shapeBookingsToTimelineItems,
} from "../../../api/bookingApi";
...
export default function ReceptionTimeline({ hotelId: hotelIdProp }) {
  ...
  const timeEnd = useMemo(
    () => currentStart.clone().add(6, "days").endOf("day"),
    [currentStart],
  );

  const hotelId = localStorage.getItem("hotelId");
  console.log("ReceptionTimeline hotelId:", hotelId);
  ...
  const handleSubmitCheckIn = async ({ payAmount, note }) => {
    ...
    try {
      ...
      if (payAmount && payAmount > 0) {
        await api.post(
          `/api/reception/bookings/${id}/payments`,
          {
            method: "OFFLINE_CASH",
            amount: payAmount,
            note: note || "Thanh toán khi nhận phòng",
      ...
      // 2) Gọi API check-in
      await api.post(`/api/reception/bookings/${id}/checkin`);
      ...
      console.error(e);
      message.error(
        e.response?.data?.error || e.message || "Lỗi khi nhận phòng",
      );
    } finally {
      setCheckInLoading(false);
    }
  ...
  const handleSubmitCheckOut = async ({ payAmount, note }) => {
    ...
    try {
      ...
      // 2) Gọi API check-out
      await api.post(`/api/reception/bookings/${id}/checkout`);
      ...
      console.error(e);
      message.error(
        e.response?.data?.error || e.message || "Lỗi khi trả phòng",
      );
    } finally {
      setCheckOutLoading(false);
    }
  ...
  const handleCleanBooking = async (booking) => {
    try {
      ...
      // Gọi API xóa mềm (soft-delete)
      await api.delete(`/api/reception/bookings/${id}`);
      ...
      console.error(e);
      message.error(
        e.response?.data?.error || e.message || "Lỗi khi dọn phòng",
      );
    }
  };
  ...
  // (có thể không dùng nữa, nhưng mình vẫn chuyển qua api cho sạch)
  const handleCheckInBooking = async (booking) => {
    try {
      const id =
        booking._id || booking.id || booking.bookingId;
  ...
  const handleSubmitEditBooking = async (body) => {
    try {
      ...
      await api.patch(
        `/api/reception/bookings/${id}/dates`,
        body,
      );
  ...
  const handleSelectItem = async (itemId) => {
    try {
      ...
      // 🔥 Gọi API chi tiết booking bằng bookingId (dùng axios)
  ...
  const goToPreviousWeek = () =>
    setCurrentStart((p) => p.clone().subtract(7, "days"));
  const goToNextWeek = () =>
    setCurrentStart((p) => p.clone().add(7, "days"));
  ...
  const handleSubmitQuickBooking = async (body) => {
    try{
      ...
      await api.post("/api/reception/bookings", body);
  ...
  const quickStats = useMemo(() => {
    ...
    rooms.forEach((r) => {
      if (!busy.has(r._id)) empty += 1;
    });

    timelineItems.forEach((it) => {
      const s = it.start_time;
      const e = it.end_time;
      if (s.diff(now, "hours") <= 24 && s.isAfter(now))
        soonCheckin += 1;
      if (now.isBetween(s, e)) inUse += 1;
      if (e.diff(now, "hours") <= 24 && e.isAfter(now))
        soonCheckout += 1;
      if (e.isBefore(now)) overdue += 1;
    });
    ...
    <div style={{ padding: 16 }}>
      ...
      >
        ...
        >
          ...
          <Tag color="green">Đang trống ({quickStats.empty})</Tag>
          <Tag color="orange">
            Sắp nhận ({quickStats.soonCheckin})
          </Tag>
          <Tag color="blue">
            Đang sử dụng ({quickStats.inUse})
          </Tag>
          <Tag color="purple">
            Sắp trả ({quickStats.soonCheckout})
          </Tag>
          <Tag color="magenta">
            Quá giờ trả ({quickStats.overdue})
          </Tag>
        </div>
        ...
        >
          ...
          >
            ...
            >
              {timeStart.format("DD/MM")} -{" "}
              {timeEnd.format("DD/MM/YYYY")}
            </Text>
            <Button
              icon={<RightOutlined />}
