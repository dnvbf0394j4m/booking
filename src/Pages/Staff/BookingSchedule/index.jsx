...
// src/pages/ReceptionBoard.jsx
import { Segmented } from "antd";
import { AppstoreOutlined, TableOutlined } from "@ant-design/icons";
import React, { useState } from "react";
import RoomGridView from "./RoomGridView";
import ReceptionTimeline from "./ReceptionTimeline"; // cái mình viết trước

export default function BookingSchedule () {
  const [mode, setMode] = useState("grid"); // grid | timeline
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 16px" }}>
        <Segmented
          ...
          onChange={setMode}
          options={[
            { label: "Sơ đồ",  value: "grid",     icon: <AppstoreOutlined /> },
            { label: "Lưới thời gian", value: "timeline", icon: <TableOutlined /> },
          ]}
        />
        {/* chỗ khác: nút Đặt phòng global, bộ lọc chung... */}
      </div>

      {mode === "grid" ? <RoomGridView /> : <ReceptionTimeline />}
    </div>
  );
}
